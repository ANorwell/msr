...
class Measurement:
    ...
    def average_response_time(self, urls):
        ...
        return ((domain, sum(response_times, datetime.timedelta()) /
                 len(response_times))
                for (domain, response_times) in by_domain.items())

    def __async(self, urls, operation):
        with futures.ThreadPoolExecutor(
                max_workers=self.parallelism) as executor:
            pairs = [(url,
                      executor.submit(lambda: operation(requests.get(url))))
                     for url in urls]

        return ((url, future.result()) for (url, future) in pairs)
